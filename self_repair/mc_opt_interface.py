...
class ModelCheckerInterface(MC_OPT_INTERFACE):
    ...
    def mc_results_from_configs(self, new_configs: pd.DataFrame, cache_file_name: str = "null") -> pd.DataFrame:
        ...
        if not self.skip_cache:
            try:
                ...
                # result.to_csv(f"mc_fix.csv", index=False)
    ...
    def opt_optimization(self, configs_to_optimize: pd.DataFrame, scs_regressor, cache_file_name: str, skip_caching=False) -> pd.DataFrame:
        ...
        if not skip_caching:
            try:
                ...
                result = optimize_configurations(configs_to_optimize.reset_index(drop=True), scs_regressor)
                result.to_csv(cache_path, index=False)
        else:
